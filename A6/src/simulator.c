	...
	while ((mainEnv->clock < mainEnv->totalTime) || customersPresent) {
		if (actionMode == customerMode) {
			...
			*(createdArgV+1) = malloc(7*sizeof(char));
			...
			for (int i = 0; i < mainEnv->numTellers; i++) {
...
simTime singleQueueSystem() {
	...
	while ((mainEnv->clock < mainEnv->totalTime) || customersPresent) {
		if (actionMode == customerMode) {
			...
			*createdArgV = malloc(7*sizeof(char));
			*(createdArgV+1) = malloc(7*sizeof(char));
			sprintf(*createdArgV, "%d", 0);
			sprintf(*(createdArgV+1), "%d", latestCustomer->customerId);

			pushFQueue(mainEnv->startQueues+0, createNode(createEvent(&serveCustomer, 2, createdArgV)));
			latestCustomer->lineIndex = 0;

			debugPrintf("%lf: customer %d entered queue %d\n", mainEnv->clock, latestCustomer->customerId, 0);
			...
			for (int i = 0; i < mainEnv->numTellers; i++) {
				if (doubleEquality(*(mainEnv->queueBusyTime+i), resumeTime)) {
					...
					// Start Queue
					busyFor = 0.0;
					node * poppedSqNode = popFQueue(mainEnv->startQueues+0);
					event * sqEve = poppedSqNode->eve;
					// Overwriting the teller assigned
					sprintf(*(sqEve->argVector), "%d", i);
					busyFor = (sqEve->eventFunction) (sqEve->argVector);
					*(mainEnv->queueBusyTime+i) = mainEnv->clock + busyFor;
					// Only working tellers reach this
					if (*(mainEnv->queueBusyTime+i) > timeRequiredToServe) timeRequiredToServe = *(mainEnv->queueBusyTime+i);
		...
		if ((mainEnv->startQueues+0)->length > 0) {
			customersPresent = 1;
		}
...
int main(int argc, char * argv[]) {
	...
	createEnv(multiQueueEnv, numTellers, numTellers, avgTellerServiceTime, numCustomers, totalTime);
	...
	simTime tellersServiceTime = 0.0;
	simTime tellerIdleTime = 0.0;
	...
	// Single Queue
	environment * singleQueueEnv = (environment *) malloc(sizeof(environment));
	createEnv(singleQueueEnv, 1, numTellers, avgTellerServiceTime, numCustomers, totalTime);
	mainEnv = singleQueueEnv;
	
	debugPrintf("\n\nInitialized of singlequeue system\n");
	timeRequired = singleQueueSystem();

	printf("\nSINGLE QUEUE\n\n");
	printf("Number of Customers served: %d\n", numCustomers);
	printf("Total time required to serve customers: %lf minutes\n", timeRequired);
	printf("Number of Tellers: %d\n", numTellers);
	printf("All tellers serve from the same single queue\n");

	meanTimeSpentCustomer = 0.0;
	for (int i = 0; i < numCustomers; i++) {
		meanTimeSpentCustomer += (mainEnv->customers+i)->exitTime - (mainEnv->customers+i)->enterTime;
	}
	meanTimeSpentCustomer /= numCustomers;

	stddevTimeSpentCustomers = 0.0;
	for (int i = 0; i < numCustomers; i++) {
		stddevTimeSpentCustomers += pow(((mainEnv->customers+i)->exitTime - (mainEnv->customers+i)->enterTime) - meanTimeSpentCustomer, 2);
	}
	stddevTimeSpentCustomers /= numCustomers;
